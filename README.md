...
The session anchors a managed footer at the bottom of the terminal: a bordered input box with a status bar underneath, repainted in place while output commits above it. The input box is a full line editor: cursor movement, Home/End, word jumps with Ctrl+Left/Right, kill bindings (Ctrl+U, Ctrl+K, Ctrl+W), and Up/Down history. Typing `/` opens a command palette that filters as you type, and Tab completes commands, option names, profile names, plugin ids, command names, and device serials. Escape clears the line, Ctrl+C twice (or Ctrl+D on an empty line) leaves the session. While a command runs, the box gives way to a live activity line with a spinner and elapsed time, and `listen` shows a live event counter in the footer until Escape stops it, without ending the session.
