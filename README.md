...
The session is a terminal application built on [Terminal.Gui](https://github.com/gui-cs/Terminal.Gui): a scrolling transcript fills the window, an input line is anchored at the bottom, and a status bar shows the key shortcuts along with the connected device count. The input line completes as you type through a popup fed by the deck command surface: commands and subcommands, `write`'s options, profile names, plugin ids, command class names, and device serials. Up/Down recall history, Escape clears the line or stops a streaming `listen` without ending the session, and Ctrl+Q leaves. Commands run on a background worker, so the interface stays responsive while `listen` streams events into the transcript.
